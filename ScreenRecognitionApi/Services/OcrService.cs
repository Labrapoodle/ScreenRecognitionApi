...
public class OcrService
{
    ...
    public async Task<string> Process(byte[] imageBytes, string fileKey)
    {
        ...
        // 1. Проверяем, существует ли шаблон на диске
        if (File.Exists(TemplatePath))
        {
            // Шаблон есть: выпрямляем картинку с помощью SIFT
            byte[] warpedImageBytes = ExtractAndWarpArea(imageBytes, TemplatePath);

            if (warpedImageBytes == null)
                throw new Exception("Не удалось выполнить выравнивание изображения по шаблону.");

            // Нарезаем выпрямленную картинку на зоны, если для этого ключа есть координаты
        ...
        {
            // ШАБЛОНА НЕТ: Пропускаем SIFT и нарезку. 
            // Отправляем исходную картинку в нейросеть целиком.
            blocksToSend.Add(imageBytes);

            // Опционально: можно вывести сообщение в консоль сервера для отладки
            Console.WriteLine($"[WARNING] Шаблон не найден по пути {TemplatePath}. Картинка обрабатывается целиком.");
        }

        // 2. Дальше идет ваш стандартный цикл отправки блоков в Ollama...
        ...
        foreach (var block in blocksToSend)
        {
            ...
            var payloadObject = new
            {
                ...
                stream = false,
                options = new { num_predict = 2048, num_ctx = 4096 }
            };
        ...
        // 3. Собираем финальный JSON с помощью вашего Regex
